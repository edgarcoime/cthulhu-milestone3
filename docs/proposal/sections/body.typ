...
= Project Summary 
CTHULHU is an Anonymous-first file sharing platform designed to address privacy and usability shortcomings in modern file sharing systems. The application will enable users to upload and share files/folders of up to 1GB with automatic deletion after 48 hours for anonymous users. Authenticated users will have more fine grain control to extend file lifetimes up to 14 days. Built using modern microservice architecture, CTHULHU will use React NEXT for the frontend and Golang for backend services, it will use HTTP as the communication layer between the frontend web application and API gateway, with gRPC for direct communication between backend services and AWS SQS (piped to Lambda) for asynchronous messaging such as notifications. This architecture ensures scalability, maintainability, and robust security while providing a seamless user experience without traditional account barriers.
...
The system will also be integrating advanced microservice architecture utilizing gRPC for direct inter-service communication and AWS SQS for asynchronous messaging when a message broker is needed. Microservices is an exciting paradigm and will enable the platform to grow according to its needs. It allows us to add additional features (i.e.. Logging service, notification service, virus scanning service, etc.) without radically changing the codebase, which Monolithic architectures would demand given the same change.


= Complexity
Building CTHULHU will involve intensive research on microservice architecture and overcoming the challenges that I have never encountered in any of my previous projects. CTHULHU will demand an understanding of distributed systems and how to effectively manage the asynchronous inter-service communication that comes from gRPC for direct calls and AWS SQS and Lambda for asynchronous workflows. It would require implementing robust gRPC contracts and SQS/Lambda integration, ensuring data consistency across multiple independent services, and making sure that data remains ACID compliant during transactions that may need to be rolled back if catastrophic failure occurs.Event driven architecture is also something that I have never had experience with and is quite infamous for its difficulty. It would require overcoming complex service orchestration challenges and understanding how to handle transactions through CAP theorem and SAGA patterns.

File upload functionality also introduces critical security vulnerabilities that require mitigation strategies for: preventing XSS attacks through malicious file uploads, implementing file type validation and sanitation, defending against path traversal attacks, and many more. The technical complexity starts to balloon when we account for the gRPC and AWS SQS–based messaging architecture, distributed system fault tolerance, and malware detection integration. Therefore, the complexity for CTHULHU I feel is more than sufficient for this major project.

= Technical Challenges
Most of the applications I have built have been primarily monolithic architectures with the occasional mix of microservice through client and server. CTHULHU gives me the opportunity to fully dive into the inner workings of microservice architecture applications especially when incorporating gRPC for direct service communication and AWS SQS with Lambda for asynchronous messaging. Through this project I will have to learn about microservice methodologies and communication patterns such as event stream pub/sub, event sourcing, Command Query Responsibility Segregation (CQRS), and the SAGA pattern for distributed transaction management.
...
All microservices within CTHULHU will seamlessly interact and coordinate as if operating within a monolithic architecture. By leveraging gRPC for direct coordination and AWS SQS for asynchronous workflows, and implementing the SAGA pattern, the system will maintain reliable communication and coherent workflows across distributed services. This approach enables the platform to support ACID-like transactional consistency within an inherently asynchronous and decoupled environment, ensuring data integrity while preserving the benefits of scalability and fault isolation.
...
- *Communication and Messaging Mastery:* Using gRPC for direct inter-service communication and AWS SQS with Lambda for asynchronous messaging introduces significant technical sophistication. This requires understanding gRPC contracts and streaming, as well as SQS queue design, Lambda triggers, and reliable async processing for use cases such as notifications.
...
- *Duration:* 4 weeks
- *Effort:* _Moderate_ — Requires implementing reliable background job processing using gRPC to facilitate communication across services and data propogation.
- *Details:*
  ...
  - AWS SQS–based event system for file lifecycle notifications, with messages piped to a Lambda function for processing.
...
- *Duration:* 6 weeks
- *Effort:* _High_ — File scanning requires deep integration with ClamAV antivirus engine, event-driven flows using AWS SQS and Lambda where needed, and careful orchestration of asynchronous workflows across multiple services.
- *Details:*
