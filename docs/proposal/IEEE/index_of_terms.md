...
| **Event-Driven Architecture** | A software architecture paradigm promoting the production, detection, consumption of, and reaction to events, utilized by Cthulhu via AWS SQS and Lambda to decouple services for asynchronous workflows such as notifications. |
...
| **gRPC** | A high-performance RPC framework used by Cthulhu for direct, synchronous communication between backend microservices. |
| **AWS SQS** | Amazon Simple Queue Service; a managed message queue used by Cthulhu for asynchronous workflows (e.g., notifications), with messages processed by Lambda functions. |
